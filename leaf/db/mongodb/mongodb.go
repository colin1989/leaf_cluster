...
func DialWithTimeout(url string, sessionNum int, dialTimeout time.Duration, timeout time.Duration) (*DialContext, error) {
	...
	c.sessions[0] = &Session{nil, 0, 0}
	for i := 1; i < sessionNum; i++ {
		c.sessions[i] = &Session{nil, 0, i}
	}
...
// goroutine safe
func (c *DialContext) RefNo() *Session {
	c.Lock()
	s := c.sessions[0]
	if s.ref == 0 {
		s.Refresh()
...
// goroutine safe
func (c *DialContext) UnRefNo(s *Session) {
	c.Lock()
	s.ref--
